...
(async () => {
    const fs = require('fs'); 
    exec(`rm kursmatauang.txt`, function(err, stdout, stderr) {});     
    ...
    const cekkurs1 = await page.evaluate(() => {
      const d = new Date();  
      let hh = d.getHours();  
      let mm = d.getMinutes();  
      let ss = d.getSeconds();  
    ...
    sleep(2000);
    const cekkurs2 = await page.evaluate((nilaikursusdtoidr0, waktukurs) => {
      const d = new Date();  
      let hh = d.getHours();  
      let mm = d.getMinutes();  
      let ss = d.getSeconds();  
    ...
    //   All to IDR
      data.forEach((row1) => { 
        ...
        // grab kurs mata uang 
        // Format: EcxRate;Date;IndirQuot;RatioFrom;From;DirQuot;RatioTo;To;
        nilaikursnya += mataUang1text.innerText+" ➡️ IDR : "+nilaiKurs.innerText+"\n";
      }); 
    //   // All to IDR
    ...
    var isikurs = "";
    isikurs += "*Kurs Mata Uang*\n";
    isikurs += "Diperbarui " + waktukursmatauangnew + "\n\n";
    isikurs += cekkurs1 + cekkurs2;
    isikurs += "\n*Kurs Pajak*\n";
    isikurs += "Diperbarui " + waktukurspajaknew + "\n\n";
    isikurs += cekkurspajak;
    fs.appendFile('kursmatauang.txt', isikurs, function (err) {
      if (err) throw err; 
      console.log(isikurs);
      sleep(2000);
      process.exit(1);
